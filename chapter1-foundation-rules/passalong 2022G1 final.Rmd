...
rsavg_summary.pass.individ_2022G1 <- pass_doner_all_yes_rsavg %>% group_by(actor_2) %>%
  dplyr::summarize (
    count =n())

rsavg_summary.pass.individ_2022G1
...
pass_nofollow_rsavg <- setdiff(unique(t2022_aggression_onlyG1$actor), unique(pass_doner_all_yes_rsavg$actor_2)
 )

#Calculate Richness score for rule following
#DO FIRST BEFORE ADDING ZEROS AND DONT DO AGAIN WITHOUT RUNNING ABOVE CODE
pass_richness_rsavg <- length(rsavg_summary.pass.individ_2022G1$actor_2)/length((unique(t2022_aggression_onlyG1$actor)
))
...
rsavg_summary.pass.individ_2022G1 <- rbind(rsavg_summary.pass.individ_2022G1, pass_nofollow_rsavg_zeros)




# Plot
individ_pass_plot_2022G1 <- ggplot(rsavg_summary.pass.individ_2022G1, aes(x = actor_2, y = count )) + 
  geom_bar(stat = "identity", width = 0.6) + # Consistent bar width
...
#Calculate eveness metric
pass_for_even_rsavg <- subset(rsavg_summary.pass.individ_2022G1, select = c(count))
...
masterframe_passalong_rsavg.df_2022G1 <- data.frame(runIDs = character(),
                             actor = character(),
                             subject = character(),
                             date = character(),
                             full_time = integer(),
...
#put each runs data into teh masterframe
  masterframe_passalong_rsavg.df_2022G1 <- rbind(masterframe_passalong_rsavg.df_2022G1, passalong_complete)
  
}

#write.csv(masterframe_passalong_rsavg.df_2022G1, "C:/Users/Xmanf/Desktop/Ch1 TDR files/pass2022G1countmaster_rsavg.csv")
...
```{r}
#masterframe_passalong_rsavg.df_2022G1 <- read.csv("C:/Users/Xmanf/Desktop/Ch1 TDR files/pass2022G1countmaster_rsavg.csv", header=TRUE, stringsAsFactors=FALSE)
...
#put run #s in an ascending vector
cum_run_IDS <- unique(masterframe_passalong_rsavg.df_2022G1$runIDs)
...
for (j in cum_run_IDS) {
  len_of_passalongs <- length(which(masterframe_passalong_rsavg.df_2022G1$rule_followed == "yes" & 
                                        masterframe_passalong_rsavg.df_2022G1$runIDs == j))
...
pass_2022G1_count_plot <- ggplot(rand_pass_rsavg.frame, aes(x=random_times_passalong_followed)) + 
  geom_histogram(aes(y=..density..),      
                 binwidth=1,
                 colour="black", fill="grey") +
...
#calculate p value (proportion of randomized reference values that are more "extreme" than the observed count of rule follows)
pass_tally_rsavg_2022G1 <-length(which(pass_real_rsavg < rand_pass_rsavg.frame$random_times_passalong_followed  ))

ppass_tally_rsavg_2022G1 <-   1 - pass_tally_rsavg_2022G1/length(rand_pass_rsavg.frame$random_times_passalong_followed)
ppass_tally_rsavg_2022G1
#p = 0
...
confint_pass_2022G1
#(Intercept)          5.47  5.728
#data_typerandomized -0.09  0.132
#tukey
